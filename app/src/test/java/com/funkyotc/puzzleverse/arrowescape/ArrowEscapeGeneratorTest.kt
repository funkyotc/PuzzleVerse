...
class ArrowEscapeGeneratorTest {
    ...
    fun updatePregeneratedFile() {
        ...
        // Expert Puzzles (61-80, 40x40)
        for (i in 1..20) {
            val random = Random(400 + i)
            val arrows = generator.generate(40, 40, 0.38f, random)
            sb.append("object ArrowEscapePuzzle$puzzleIndex {\n")
            sb.append("    val arrows = listOf<Arrow>(\n")
            arrows.forEach { a ->
                val segs = a.segments.joinToString(", ") { "Coordinate(${it.x}, ${it.y})" }
        ...
        // Master Puzzles (81-100, 50x50)
        for (i in 1..20) {
            val random = Random(500 + i)
            val arrows = generator.generate(50, 50, 0.35f, random)
            sb.append("object ArrowEscapePuzzle$puzzleIndex {\n")
            sb.append("    val arrows = listOf<Arrow>(\n")
            arrows.forEach { a ->
                val segs = a.segments.joinToString(", ") { "Coordinate(${it.x}, ${it.y})" }
        ...
        println("Successfully updated ArrowEscapePregenerated.kt with 100 puzzles (${file.length()} bytes)!")
    }
}
