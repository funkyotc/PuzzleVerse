package com.funkyotc.puzzleverse.arrowescape.tools
...
fun main(args: Array<String>) {
    ...
    println("================================================================Raw")
    println("ARROW ESCAPE HEAVY OPTIMIZATION GENERATOR")
    ...
    for (slot in slots) {
        ...
        // Logger thread for live terminal updates
        ...
        val loggerThread = Thread {
            ...
            while (loggerActive.get() == 1) {
                ...
                System.out.printf(
                    "\r[%s %03d/020] Iter %,d/%,d (%s%%) | Best Q: %s | Depth: %s | Starters: %s | Rate: %,d c/s | Elapsed: %02d:%02d",
        ...
        if (finalBest == null) {
            // Fallback generation if candidates budget didn't hit criteria
            println("\n[Extreme ${String.format("%03d", slot.slotNumber)}] Standard single candidate fallback...")
