...
| Pull the Pin | All 48 levels provide a matching colored ball for each cup | A winning pin order and physics capture remain unproven for all 48 |
...
The no-shuffle Hexa Sort probe leaves `Easy_001`, `_004`, `_008`, `Medium_001`, `_002`, `_004`, `_006`–`_008`, and all eight Hard IDs unproven. It excludes the game's two shuffle actions, so these are proof gaps, not established impossible boards. Pull the Pin's 48 levels and Wood Screws' 50 levels still need their physics-based final-win proof.
